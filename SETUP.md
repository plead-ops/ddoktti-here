...
2. ⬜ 입력창의 **JSON/YAML 토글을 확인**하고 같은 형식을 붙여넣은 뒤 **Create**:
   - **JSON 탭(기본)** → [`slack-app-manifest.json`](./slack-app-manifest.json)
   - YAML 탭 → [`slack-app-manifest.yml`](./slack-app-manifest.yml)
   - ⚠️ JSON 탭에 YAML(특히 `#` 주석)을 붙여넣으면 **문법 오류**가 납니다. 형식을 맞추세요.
